...
export default function AdminDashboard() {
  ...
  const fetchUsers = async (s = '') => {
    ...
    try {
      const { data } = await api.get('/admin/users', { params: { search: s, limit: 50 } });
      setUsers(data.users);
  ...
  const fetchPosts = async () => {
    ...
    try {
      const { data } = await api.get('/admin/posts', { params: { limit: 50 } });
      setPosts(data.posts);
  ...
  return (
    <div className="min-h-screen bg-orange-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-gray-500 mb-8">Platform overview — stats are demo data, users and posts are live</p>
        ...
        {tab === 'overview' && (
          <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div key={d._id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{d.totalDonations} donations · {d.rating} rating</div>
                      </div>
                    </div>
                  ...
                  <div key={n._id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{n.totalPickups} pickups · {n.rating} rating</div>
                      </div>
                    </div>
            ...
            <div className="card md:col-span-2">
              <h3 className="font-semibold text-gray-800 mb-4">Platform Breakdown</h3>
        ...
        {tab === 'users' && (
          <div className="card">
            <input className="input mb-5" placeholder="Search by name, email or organization..."
              value={search}
              onChange={e => { setSearch(e.target.value); fetchUsers(e.target.value); }} />
            {loadingUsers ? (
              <div className="flex justify-center py-10"><div className="animate-spin h-6 w-6 rounded-full border-4 border-brand-500 border-t-transparent" /></div>
            ) : (
                    ...
                    {filteredUsers.map(u => (
                      <tr key={u._id} className="hover:bg-gray-50">
                        ...
                        <td className="py-2.5 pr-4">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${u.role === 'ngo' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>
                            {u.role}
                          </span>
                        </td>
