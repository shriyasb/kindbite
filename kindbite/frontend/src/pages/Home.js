...
const steps = [
  { step: '1', title: 'Donor Posts Food', desc: 'Restaurants, homes, and events upload surplus food with location and expiry time.', icon: '📦' },
  { step: '2', title: 'NGO Gets Notified', desc: 'Nearby NGOs receive real-time alerts and see the food on the map.', icon: '🔔' },
  { step: '3', title: 'Pickup and Delivery', desc: 'A volunteer picks up the food and marks delivery. Meals saved, waste eliminated.', icon: '🚗' },
];
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-16 px-4 bg-orange-50">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {steps.map(s => (
              <div key={s.step} className="card text-center hover:shadow-lg transition-all">
      ...
      <section className="py-16 px-4 bg-brand-500 text-white text-center">
        ...
        <Link to="/register" className="bg-white text-brand-600 font-semibold px-8 py-3 rounded-lg hover:bg-orange-50 transition inline-block">
          Create Free Account
        </Link>
      </section>

      <footer className="bg-gray-900 text-gray-400 text-sm text-center py-6">
        © {new Date().getFullYear()} KindBite — Built to eliminate food waste
      </footer>
    </div>
  );
}
