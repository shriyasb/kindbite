...
export default function PostFood() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.lat || !form.lng) return toast.error('Please provide pickup location');
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-orange-50 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">Help reduce waste by sharing your surplus food with those in need.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-5">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <div className={`relative w-10 h-5 rounded-full transition ${form.isUrgent ? 'bg-red-500' : 'bg-gray-200'}`}
