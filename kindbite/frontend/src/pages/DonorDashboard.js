      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Meals Saved', value: user?.totalMealsSaved || 0, icon: '🍽️', color: 'text-green-600' },
            { label: 'Rating', value: user?.rating ? `${user.rating.toFixed(1)} ⭐` : 'No ratings yet', icon: '⭐', color: 'text-yellow-600' },
        ...
        {loading ? (
          ...
          <div className="card text-center py-16">
            ...
            <p className="text-gray-500 mb-4">No donations yet. Start by posting surplus food.</p>
            <Link to="/post-food" className="btn-primary inline-block">Post Food Now</Link>
          </div>
        ) : (
                ...
                <div className="flex flex-wrap gap-3 items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 text-sm text-gray-500 mb-2">
                      ...
                      <span>{post.address?.slice(0, 50)}</span>
                    </div>
                    {post.acceptedBy && (
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {post.status === 'available' && (
                      <button onClick={() => cancelPost(post._id)} className="text-xs text-red-500 hover:underline">Cancel post</button>
                    )}
                  </div>
                </div>
              </div>
